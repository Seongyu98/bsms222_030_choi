...
3x2+2x−1 , we can copy and paste the code above and then redefine the variables and recompute the solution:
```{r}
a <- 3
b <- 2
c <- -1
...
## 2.3 Exercises
1. What is the sum of the first 100 positive integers? The formula for the sum of integers 1 through n is n(n+1)/2. Define n=100 and then use R to compute the sum of 1through 100 using the formula. What is the sum?
```{r}
n<-100
n*(n+1)/2
```
...
4. In math and programming, we say that we evaluate a function when we replace the argument with a given number. So if we type sqrt(4), we evaluate the sqrt function. In R, you can evaluate a function inside another function. The evaluations happen from the inside out. Use one line of code to compute the log, in base 10, of the square root of 100.sqrt(4) sqrt
```{r}
sqrt(4)
sqrt(100)
```
...
## 2.4 Data types
Variables in R can be of different types. For example, we need to distinguish numbers from character strings and tables from simple lists of numbers. The function helps us determine what type of object we have:class
```{r}
a <- 2
class(a)
```
...
A large proportion of data analysis challenges start with data stored in a data frame. For example, we stored the data for our motivating example in a data frame. You can access this dataset by loading the dslabs library and loading the dataset using the function: murders data
```{r}
library(dslabs)
data(murders)
```
...
The function is useful for finding out more about the structure of an object: str
```{r}
str(murders)
```
...
murders$population
```
But how did we know to use ? Previously, by applying the function to the object , we revealed the names for each of the five variables stored in this table. We can quickly access the variable names using: population str murders
```{r}
names(murders)
```
It is important to know that the order of the entries in preserves the order of the rows in our data table. This will later permit us to manipulate one variable based on the results of another. For example, we will be able to order the state names by the number of murders.murders$population

Tip: R comes with a very nice auto-complete functionality that saves us the trouble of typing out all the names. Try typing then hitting the tab key on your keyboard. This functionality and many other useful auto-complete features are available when working in RStudio. murders$p

## 2.4.4 Vectors: numerics, characters, and logical
The object is not one number but several. We call these types of objects vectors. A single number is technically a vector of length 1, but in general we use the term vectors to refer to objects with several entries. The function tells you how many entries are in the vector:murders$population length
```{r}
...
Another important type of vectors are logical vectors. These must be either or .TRUEFALSE
```{r}
z <- 3 == 2
z
class(z)
...
Advanced: Mathematically, the values in are integers and there is an integer class in R. However, by default, numbers are assigned class numeric even when they are round integers. For example, returns numeric. You can turn them into class integer with the function or by adding an like this: . Note the class by typing:  pop class(1) as.integer() L 1L class(1L)

## 2.4.5 Factors
In the dataset, we might expect the region to also be a character vector. However, it is not: murders
```{r}
class(murders$region)
```
It is a factor. Factors are useful for storing categorical data. We can see that there are only 4 regions by using the function: levels
```{r}
...
Note that the levels have an order that is different from the order of appearance in the factor object. The default in R is for the levels to follow alphabetical order. However, often we want the levels to follow a different order. You can specify an order through the argument when creating the factor with the function. For example, in the murders dataset regions are ordered from east to west. The function lets us change the order of the levels of a factor variable based on a summary computed on a numeric vector. We will demonstrate this with a simple example, and will see more advanced ones in the Data Visualization part of the book.levels factor reorder

Suppose we want the levels of the region by the total number of murders rather than alphabetical order. If there are values associated with each level, we can use the and specify a data summary to determine the order. The following code takes the sum of the total murders in each region, and reorders the factor following these sums. reorder
```{r}
...
## 2.4.6 Lists
Data frames are a special case of lists. Lists are useful because you can store any combination of different types. You can create a list using the function like this:list
```{r}
record <- list(name = "John Doe",
             ...
             final_grade = "A")
```
The function is described in Section 2.6.c
...
We can define a matrix using the function. We need to specify the number of rows and columns.matrix
```{r}
mat <- matrix(1:12, 4, 3)
mat
```
...
data(murders)
```
Use the function to examine the structure of the object. Which of the following best describes the variables represented in this data frame? str murders
```{r}
str(murders)
```
...
3. Use the accessor to extract the state abbreviations and assign them to the object . What is the class of this object? $ a
```{r}
class(murders$abb)
a<-murders$abb
```

4. Now use the square brackets to extract the state abbreviations and assign them to the object . Use the function to determine if and are the same. 
b identical a b
```{r}
b<-murders[["abb"]]
identical(a,b)
...
5. We saw that the column stores a factor. You can corroborate this by typing: region
```{r}
class(murders$region)
```
With one line of code, use the function and to determine the number of regions defined by this dataset.levels length
```{r}
levels(murders$region)
length(murders$region)
```

6. The function takes a vector and returns the frequency of each element. You can quickly see how many states are in each region by applying this function. Use this function in one line of code to create a table of states per region. table
```{r}
...
We can create vectors using the function , which stands for concatenate. We use to concatenate entries in the following way:cc
```{r}
codes <- c(380, 124, 818)
codes
```
...
country <- c(italy, canada, egypt)
```
you receive an error because the variables , , and are not defined. If we do not use the quotes, R looks for variables with those names and returns an error.italy canada egypt
...
codes
```
The object continues to be a numeric vector:codes
```{r}
class(codes)
```
but with names:
...
We can also assign names using the functions:names
```{r}
codes <- c(380, 124, 818)
country <- c("italy","canada","egypt")
names(codes) <- country
...
## 2.6.4 Subsetting
We use square brackets to access specific elements of a vector. For the vector we defined above, we can access the second element using: codes
```{r}
codes[2]
```
...
x <- c(1, "canada", 3)
```
But we don’t get one, not even a warning! What happened? Look at and its class:x
```{r}
x
class(x)
```
R coerced the data into characters. It guessed that because you put a character string in the vector, you meant the 1 and 3 to actually be character strings and “.” The fact that not even a warning is issued is an example of how coercion can cause many unnoticed errors in R."1"3
...
When a function tries to coerce one type to another and encounters an impossible case, it usually gives us a warning and turns the entry into a special value called an for “not available.” For example:NA
```{r}
x <- c("1", "b", "3")
as.numeric(x)
```
...
## 2.8 Exercises
1. Use the function to create a vector with the average high temperatures in January for Beijing, Lagos, Paris, Rio de Janeiro, San Juan, and Toronto, which are 35, 88, 42, 84, 81, and 30 degrees Fahrenheit. Call the object .c temp
```{r}
...
3. Use the function and the objects defined in the previous exercises to associate the temperature data with its corresponding city. names
```{r}
...
8. Create a vector of numbers that starts at 6, does not pass 55, and adds numbers in increments of 4/7: 6, 6 + 4/7, 6 + 8/7, and so on. How many numbers does the list have? Hint: use and .seq length
```{r}
length(seq(6,55,4/7))
```
...
11. The class of is numeric, not integer. R defaults to numeric and to force an integer, you need to add the letter . Confirm that the class of is integer.class(a<-1) L 1L
```{r}
class(a<-1L)
```
...
## 2.9.1 sort
Say we want to rank the states from least to most gun murders. The function sorts a vector in increasing order. We can therefore see the largest number of gun murders by typing:sort
```{r}
library(dslabs)
data(murders)
sort(murders$total)
...
## 2.9.2 order
The function is closer to what we want. It takes a vector as input and returns the vector of indexes that sorts the input vector. This may sound confusing so let’s look at a simple example. We can create a vector and sort it:order
```{r}
x <- c(31, 4, 15, 92, 65)
sort(x)
```
Rather than sort the input vector, the function returns the index that sorts input vector: order
```{r}
index <- order(x)
x[index]
```
This is the same output as that returned by . If we look at this index, we see why it works: sort(x)
```{r}
x
order(x)
```
The second entry of is the smallest, so starts with . The next smallest is the third entry, so the second entry is and so on.x order(x)2 3
...
## 2.9.3 and maxwhich.max
If we are only interested in the entry with the largest value, we can use for the value:max
```{r}
max(murders$total)
```
and for the index of the largest value: which.max
```{r}
i_max <- which.max(murders$total)
murders$state[i_max]
```
For the minimum, we can use and in the same way.     min which.min
...
## 2.9.4 rank
Although not as frequently used as and , the function is also related to order and can be useful. For any given vector it returns a vector with the rank of the first entry, second entry, etc., of the input vector. Here is a simple example:order sort rank
```{r}
x <- c(31, 4, 15, 92, 65)
rank(x)
```
...
x+y
```
We do get a warning, but no error. For the output, R has recycled the numbers in . Notice the last digit of numbers in the output.x
...
data("murders")
```
1. Use the operator to access the population size data and store it as the object. Then use the function to redefine so that it is sorted. Finally, use the operator to report the smallest population size.$ pop sort pop [
```{r}
pop<-murders$population
sort(pop)
min(pop)
...
2. Now instead of the smallest population size, find the index of the entry with the smallest population size. Hint: use instead of .order sort
```{r}
...
3. We can actually perform the same operation as in the previous exercise using the function . Write one line of code that does this. which.min
```{r}
which.min(murders$population)
```

4. Now we know how small the smallest state is and we know which row represents it. Which state is it? Define a variable to be the state names from the data frame. Report the name of the state with the smallest population.states murders
```{r}
pop<-murders$population
...
5. You can create a data frame using the function. Here is a quick example: data.frame
```{r}
temp <- c(35, 88, 42, 84, 81, 30)
...
```
Use the function to determine the population rank of each state from smallest population size to biggest. Save these ranks in an object called , then create a data frame with the state name and its rank. Call the data frame .rank ranks my_df
```{r}
rank(murders$population)
ranks<-rank(murders$population)
my_df<-data.frame(name=murders$state,ranks=rank(murders$population))
```
...
```
and want to convert to centimeters. Notice what happens when we multiply by 2.54:inches
```{r}
inches * 2.54
```
...
3. Compute the per 100,000 murder rate for each state and store it in the object . Then compute the average murder rate for the US using the function . What is the average?murder_rate mean
```{r}
murder_rate<-murders$total/murders$population*100000
mean(murder_rate)
```
...
ind <- murder_rate <= 0.71
```
Note that we get back a logical vector with for each entry smaller than or equal to 0.71. To see which states these are, we can leverage the fact that vectors can be indexed with logicals. TRUE
```{r}
murders$state[ind]
```
In order to count how many are TRUE, the function returns the sum of the entries of a vector and logical vectors get coerced to numeric with coded as 1 and as 0. Thus we can count the states using:sum TRUE FALSE
```{r}
sum(ind)
```
## 2.13.2 Logical operators
Suppose we like the mountains and we want to move to a safe state in the western region of the country. We want the murder rate to be at most 1. In this case, we want two different things to be true. Here we can use the logical operator and, which in R is represented with . This operation results in only when both logicals are . To see this, consider this example:& TRUE TRUE
```{r}
TRUE & TRUE
TRUE & FALSE
FALSE & FALSE
...
```
## 2.13.3 which
Suppose we want to look up California’s murder rate. For this type of operation, it is convenient to convert vectors of logicals into indexes instead of keeping long vectors of logicals. The function tells us which entries of a logical vector are TRUE. So we can type: which
```{r}
ind <- which(murders$state == "California")
murder_rate[ind]
```
## 2.13.4 match
If instead of just one state we want to find out the murder rates for several states, say New York, Florida, and Texas, we can use the function . This function tells us which indexes of a second vector match each of the entries of a first vector: match
```{r}
...
```
## 2.13.5 %in%
If rather than an index we want a logical that tells us whether or not each element of a first vector is in a second, we can use the function . Let’s imagine you are not sure if Boston, Dakota, and Washington are states. You can find out like this: %in%
```{r}
c("Boston", "Dakota", "Washington") %in% murders$state
```
Note that we will be using often throughout the book. %in%

Advanced: There is a connection between and through . To see this, notice that the following two lines produce the same index (although in different order): match %in% which
```{r}
...
## 2.15.1 plot
The function can be used to make scatterplots. Here is a plot of total murders versus population. plot
```{r}
x <- murders$population / 10^6
y <- murders$total
plot(x, y)
```
For a quick plot that avoids accessing variables twice, we can use the function: with
```{r}
with(murders, plot(population, total))
```
The function lets us use the column names in the function. It also works with any data frames and any function.with murders plot
