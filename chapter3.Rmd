---
title: "Chapter3: Programming basics"
...
```{r}
A=c(0,1,2,-4,5)
TF=c(FALSE,TRUE,TRUE,FALSE,TRUE)
one=c(1/A)
two=c(NA,NA,NA,NA,NA)
rst=ifelse(A>0,1/A,NA)
w_hpnd=data.frame(a=A,is_a_positive=TF,answer1=one,answer2=two,result=rst)
w_hpnd
```
...
$1+2+⋯+n is n(n+1)/2$
 . What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the  
$Sn$ :
```{r}
...
```
In each iteration  
$n=1, n=2 , etc…$, we compute  
$Sn$  and store it in the n th entry of s_n.
...
d.all(!x)
```{r}
x_1 <- c(TRUE, FALSE, FALSE, FALSE)
x_2 <- c(TRUE, TRUE, FALSE, FALSE)
all(x_1)
...
altman_plot(c(52,44,56,5,7),c(22,15,47,1,5))
```
...
n  computes the sum $Sn=1^2+2^2+3^2+…n^2$ . Report the value of the sum when n=10.
```{r}
